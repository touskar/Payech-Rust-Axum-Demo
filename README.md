...
└── products.rs      # Product management utilities
```

## Security

- HMAC signature verification for PayTech callbacks
- Environment-based configuration for sensitive data
- Proper error handling and validation

## PayTech Integration

This implementation follows the PayTech API documentation available at [https://docs.intech.sn/doc_paytech.php](https://docs.intech.sn/doc_paytech.php)
...
### PayTech API Response Issues

If you encounter parsing errors with PayTech responses, the application provides detailed debugging:

1. **Check the logs** - The server logs show:
   - PayTech request being sent
   - HTTP status code from PayTech
   - Raw response body
   - Parsed JSON structure

2. **Common issues**:
   - Missing `token` field: The response model handles multiple possible field names
   - Invalid API credentials: Check your `PAYTECH_API_KEY` and `PAYTECH_API_SECRET`
   - Wrong environment: Ensure `PAYTECH_ENV` is set correctly (`test` or `prod`)

3. **Response field variations**: The code handles multiple possible field names:
   - Payment URL: `redirect_url`, `payment_url`, `url`, `redirect`
   - Payment Token: `token`, `payment_token`

### Example Debug Output

```
Sending PayTech request: {
  "item_name": "Achat de 2 article(s)",
  "item_price": 297500,
  "ref_command": "CMD_1703123456",
  "currency": "XOF",
  "env": "test"
}
PayTech API HTTP Status: 200 OK
PayTech API Response: {"success":1,"redirect_url":"https://...","token":"abc123"}
Parsed JSON structure: {
  "success": 1,
